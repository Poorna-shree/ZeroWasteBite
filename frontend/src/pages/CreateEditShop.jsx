...
function CreateEditShop() {
    ...
    // Populate state from existing shop data (or user defaults) on mount
    useEffect(() => {
        if (myShopData) {
            ...
            <div className='max-w-lg w-full bg-white shadow-xl rounded-2xl p-8 border border-orange-100'>
                ...
                <form className='space-y-5' onSubmit={handleSubmit}>
                    ...
                    <div>
                        ...
                        {frontendImage && (
                            <div className='mt-4'>
                                <img src={frontendImage} alt="Shop Front" className='w-full h-48 object-cover rounded-lg border' />
                            </div>
                        )}
                    </div>
                    ...
                    <button className='w-full bg-[#ff4d2d] text-white px-6 py-3 rounded-lg font-semibold shadow-md hover:bg-orange-600 hover:shadow-lg transition-all duration-200 cursor-pointer'disabled={loading}>
                        {loading?<ClockLoader size={20} color='white'/>:"Save"}
